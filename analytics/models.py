...
def post_save_session_receiever(sender, instance, created, *args, **kwargs):
    if created:
        qs = UserSession.objects.filter(user=instance.user, ended=False, active=False).exclude(id= instance.id)
        for i in qs:
            i.end_sesion()
    if not instance.acitve and not instance.ended:
        instance.end_sesion()

if FORCE_SESSION_TO_ONE:
    post_save.connect(post_save_session_receiever, sender=UserSession)
