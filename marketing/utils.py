...
if MAILCHIMP_DATA_CENTER is None:
    raise NotImplementedError("MAILCHIMP_DATA_CENTER must be set in the settings, something like us19")

MAILCHIMP_EMAIL_LIST_ID = getattr(settings, "MAILCHIMP_EMAIL_LIST_ID", None)

if MAILCHIMP_EMAIL_LIST_ID is None:
    raise NotImplementedError("MAILCHIMP_EMAIL_LIST_ID must be set in the settings, something like us19")



def check_email(email):
    if not re.match(r".+@.+\..+", email):
        raise ValueError('Geçerli bir email adresi değil')
    return email
...
class Mailchimp(object):
    ...
    def check_valid_status(self, status):
        ...
        if status not in choices:
            raise ValueError("Geçerli bir seçim değil")
        return status
